...
"""
    ...
    def check_compatibility(self):
        ...
        if (
            self._find_existing_interpreter(python_version) is not None
    ...
    def _get_concrete_interpreter_path(self, python_version: str = '') -> str | None:
        if python_version:
            return self._resolve_concrete_interpreter_path(python_version)

        return (
            # Prefer the Python version Hatch is currently using
            self._resolve_concrete_interpreter_path(self._preferred_python_version)
            # Fallback to whatever is compatible
            or self._resolve_concrete_interpreter_path('')
        )

    def _resolve_concrete_interpreter_path(self, python_version: str) -> str | None:
    ...
    def _is_stable_path(self, executable: str) -> bool:
        ...
        if str(path).endswith('WindowsApps\\python.exe'):
            return False
